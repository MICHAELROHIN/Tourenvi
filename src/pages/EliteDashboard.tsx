  ...
  if (calcData && !calcData.success) {
    ...
    return (
      <div className="h-screen pt-16 bg-gt-offwhite flex items-center justify-center p-6">
        <div className="max-w-2xl w-full bg-white rounded-2xl shadow-xl border-t-8 border-emerald-600 p-8 md:p-12 text-center animate-in fade-in zoom-in-95 duration-300">
          ...
          <h2 className="text-3xl sm:text-4xl font-serif font-bold text-emerald-950 mb-4">Cannot estimate within the given budget.</h2>
          <p className="text-gray-600 font-sans text-base sm:text-lg mb-8 leading-relaxed">
          ...
          <div className="bg-emerald-50/60 rounded-xl p-6 border border-emerald-100 text-left mb-8 max-w-md mx-auto">
            <h3 className="font-semibold text-emerald-900 mb-4 border-b border-emerald-200/60 pb-2 text-sm uppercase tracking-wider">Itemized Cost Breakdown</h3>
            <div className="space-y-2 text-sm">
              ...
              <div className="flex justify-between border-t border-emerald-200/60 pt-2 font-bold text-emerald-950 text-base">
                <span>Total Calculated:</span>
                <span>₹{(errorDetails.totalCost || 0).toLocaleString()}</span>
              </div>
              <div className="flex justify-between font-semibold text-amber-700 text-sm">
          ...
          <button
            ...
            className="px-8 py-4 bg-emerald-600 hover:bg-emerald-700 text-white font-semibold rounded-xl shadow-lg shadow-emerald-600/20 transition-all active:scale-[0.98] inline-flex items-center gap-2 cursor-pointer"
          >
    ...
    <div className="min-h-screen lg:h-screen pt-16 bg-gt-offwhite flex flex-col lg:overflow-hidden font-sans">
      ...
      <div className="w-full flex flex-col lg:flex-row flex-1 lg:overflow-hidden">
        ...
        <div className={`w-full lg:w-1/3 p-4 sm:p-6 lg:p-8 bg-white border-r border-gray-100 lg:overflow-y-auto lg:h-full custom-scrollbar pb-24 ${mobileTab === "itinerary" ? "block" : "hidden lg:block"}`}>
          <h2 className="text-2xl sm:text-3xl font-serif font-bold text-emerald-900 mb-1.5 sm:mb-2">Bespoke Itinerary</h2>
          ...
          <div className="space-y-6 sm:space-y-8">
            {destinationShowcase.length > 0 ? (
              <div className="space-y-4 sm:space-y-5">
                <div>
                  <h3 className="text-lg sm:text-xl font-serif font-semibold text-emerald-900">Sightseeing Highlights</h3>
                ...
                {destinationShowcase.map((group: any) => (
                  <div key={group.id} className="space-y-3">
                    <div className="flex items-center justify-between">
                      <div>
                        <h4 className="font-semibold text-emerald-900 text-sm sm:text-base">{group.matchedDestination || group.destination}</h4>
                        {group.region ? <p className="text-xs text-gray-500">{group.region}</p> : null}
                      </div>
                      <span className="text-[10px] sm:text-[11px] uppercase tracking-[0.18em] text-emerald-700 font-semibold">Premium Picks</span>
                    </div>
                        ...
                        <div key={place.id} className="group overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-sm transition hover:-translate-y-0.5 hover:shadow-lg">
                          <div className="relative h-36 sm:h-40 overflow-hidden">
                            ...
                            <div className="absolute bottom-2.5 left-2.5 right-2.5 sm:bottom-3 sm:left-3 sm:right-3">
                              <span className="inline-flex rounded-full bg-white/90 px-2 py-0.5 text-[9px] sm:text-[10px] font-semibold uppercase tracking-[0.15em] text-emerald-800">
            ...
            {itineraryDays.map((day, idx) => (
              <div key={idx} className="relative">
                ...
                <h3 className="text-lg sm:text-xl font-serif font-semibold text-emerald-900 mb-3 sm:mb-4 flex items-center gap-3">
                  <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-full bg-emerald-100 text-emerald-800 flex items-center justify-center font-bold text-sm sm:text-base z-10">
                    D{day.day}
                  </div>
                  {day.title}
                </h3>
                ...
                <div className="ml-4 sm:ml-5 pl-6 sm:pl-8 space-y-4 sm:space-y-6">
                  {day.items.map((item, itemIdx) => (
                    <div key={itemIdx} className="relative group">
                      <div className="absolute -left-8 sm:-left-10 top-1 w-3.5 h-3.5 sm:w-4 sm:h-4 rounded-full border-2 border-white bg-gray-200 group-hover:bg-emerald-600 transition-colors shadow-sm"></div>
                      <div className="bg-gray-50 border border-gray-100 rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-all group-hover:border-emerald-500/30">
                        {item.image && (
                          <div className="relative h-36 sm:h-44 w-full overflow-hidden">
                            ...
                            <div className="absolute bottom-2.5 left-3 right-3 sm:bottom-3 sm:left-4 sm:right-4">
                              <span className="px-2 py-0.5 bg-emerald-600 text-white font-bold rounded text-[9px] sm:text-[10px] uppercase tracking-wider">
                        ...
                        <div className="p-3.5 sm:p-4">
                          <div className="flex items-center gap-2.5 sm:gap-3 mb-1.5 sm:mb-2">
                            {item.type === "food" || item.iconName === "Coffee" ? (
                              <Coffee size={15} className="text-emerald-600 animate-bounce-subtle" />
                            ) : item.type === "lodging" || item.iconName === "Bed" ? (
                              <Bed size={15} className="text-emerald-600 animate-bounce-subtle" />
                            ) : item.type === "sightseeing" || item.iconName === "Camera" ? (
                              <Camera size={15} className="text-emerald-600 animate-bounce-subtle" />
                            ) : (
                              <MapPin size={15} className="text-emerald-600 animate-bounce-subtle" />
                            )}
                            <span className="text-xs font-semibold text-gray-500">{item.time}</span>
                          </div>
                          <h4 className="font-semibold text-sm sm:text-base text-emerald-900 mb-1">{item.title}</h4>
                          {item.description && (
          ...
          <div className="relative h-full w-full">
            ...
            <div className="absolute bottom-3 left-3 right-3 sm:bottom-6 sm:left-6 sm:right-6 bg-white/95 backdrop-blur-md p-2.5 sm:p-4 rounded-xl shadow-lg border border-white flex justify-between items-center z-[500] pointer-events-none">
              <div className="text-center sm:text-left">
                ...
                <p className="font-bold text-emerald-900 text-sm sm:text-lg">~{mockDistanceKm} km</p>
              </div>
              ...
              <div className="text-center sm:text-left">
                ...
                <p className="font-bold text-emerald-900 text-sm sm:text-lg capitalize">{trip.vehicleType}</p>
              </div>
              <div className="h-6 sm:h-8 w-px bg-gray-200"></div>
              <div className="text-center sm:text-left">
                <p className="text-[10px] sm:text-xs text-gray-500 uppercase tracking-wide font-bold">Fuel</p>
                <p className="font-bold text-emerald-900 text-sm sm:text-lg capitalize">{trip.fuelType}</p>
              </div>
            </div>
          </div>
        </div>
