  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4 border-b border-gray-100 pb-4 mb-4 sm:mb-5">
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2 mb-2">
            <span className="px-2.5 sm:px-3 py-0.5 sm:py-1 bg-emerald-100 text-emerald-800 text-[10px] sm:text-xs font-bold uppercase tracking-wider rounded-full flex items-center gap-1.5">
          ...
          <h3 className="text-lg sm:text-2xl font-serif font-bold text-emerald-950 flex flex-wrap items-center gap-1.5 sm:gap-2 leading-snug">
            <span className="text-gray-900 break-words">{route.origin}</span>
            <ArrowRight size={18} className="text-emerald-600 shrink-0 inline-block" />
            <span className="text-emerald-800 break-words">{route.destination}</span>
          </h3>
      ...
      <div className="mb-4 sm:mb-5 bg-gray-50/70 rounded-xl p-3 sm:p-4 border border-gray-100">
        <p className="text-[11px] sm:text-xs font-bold text-emerald-900 uppercase tracking-wider mb-2.5 sm:mb-3 flex items-center gap-1.5">
          <Milestone size={14} className="text-emerald-600" /> Waypoint Transit Path (Where to Where)
        </p>
          ...
          {route.waypoints.map((wp, idx) => (
            <React.Fragment key={idx}>
              <div className="flex items-center gap-1.5 sm:gap-2 shrink-0 bg-white border border-gray-200 px-2.5 sm:px-3 py-1 sm:py-1.5 rounded-xl shadow-2xs">
                <span className="w-4 h-4 sm:w-5 sm:h-5 rounded-full bg-emerald-100 text-emerald-800 text-[9px] sm:text-[10px] font-bold flex items-center justify-center">
                  {idx + 1}
                </span>
                <span className="text-xs font-semibold text-gray-800 whitespace-nowrap">{wp}</span>
              </div>
              {idx < route.waypoints.length - 1 && (
                <ArrowRight size={13} className="text-emerald-600 shrink-0 opacity-70" />
              )}
            </React.Fragment>
          ))}
        </div>
      ...
      <div>
        <div className="flex items-center justify-between mb-2.5 sm:mb-3">
          <h4 className="text-[11px] sm:text-xs font-bold text-gray-500 uppercase tracking-wider flex items-center gap-1.5">
            <Car size={14} className="text-emerald-600" /> Leg Travel Details ({route.legs.length} Legs)
          </h4>
          <button
            ...
            {route.legs.map((leg, idx) => (
              ...
              >
                <div className="flex items-start gap-2.5 sm:gap-3 flex-1 min-w-0">
                  <span className="w-6 h-6 rounded-lg bg-emerald-100 text-emerald-800 font-bold text-[11px] flex items-center justify-center shrink-0 mt-0.5">
                    L{idx + 1}
                  </span>
                  <div className="flex-1 min-w-0">
                    <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-1.5 font-semibold text-gray-800">
                      ...
                      <div className="flex items-center gap-1 sm:hidden text-emerald-600 text-[10px] font-bold">
                        <ArrowRight size={10} /> <span>to</span>
                      </div>
                      <span className="text-emerald-900 font-bold break-words">{leg.to}</span>
                    </div>
                    {leg.highway && (
                ...
                <div className="flex items-center gap-3 text-gray-600 self-start sm:self-center font-medium bg-white px-2.5 sm:px-3 py-1 rounded-lg border border-gray-200 shadow-2xs shrink-0 text-[11px] sm:text-xs">
                  <span className="flex items-center gap-1 text-gray-700">
                    <Milestone size={12} className="text-emerald-600" /> {leg.distanceKm} km
                  </span>
                  <span className="flex items-center gap-1 text-gray-500">
                    <Clock size={12} className="text-emerald-600" /> {leg.estimatedTime}
                  </span>
                </div>
              </div>
            ))}
