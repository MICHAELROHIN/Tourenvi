...
// --------------------------- GOOGLE PLACES: Hotel Details & Photos ---------------------------
// This is now the ONLY route we will use for hotels.
app.get("/get-hotels", async (req, res) => {
  ...
  const textSearchUrl = `https://maps.googleapis.com/maps/api/place/textsearch/json?query=hotels%20in%20${encodeURIComponent(destination)}&key=${apiKey}`;
  ...
  try {
    ...
    const hotelDetailsPromises = places.map(async (place) => {
      ...
      // Fetch all fields needed for estimation: rating, price_level, photos, etc.
      const detailsUrl = `https://maps.googleapis.com/maps/api/place/details/json?place_id=${placeId}&fields=name,place_id,formatted_address,formatted_phone_number,rating,user_ratings_total,photos,price_level&key=${apiKey}`;
      try {
        ...
        return {
          ...
          photoUrl,
          price_level: details.price_level ?? null, // price_level (0-4) is crucial for estimation
        };
      } catch (detailsError) {
...
// --------------------------- AMADEUS CODE REMOVED ---------------------------
// We are not using Amadeus for prices because the Test keys do not
// have data for Indian cities. This is the only way to show prices.
// --------------------------------------------------------------------------
...
// --------------------------- SERVER START ---------------------------
app.listen(8000, () => console.log("✅ Server running on http://localhost:8000. Now using Google-only for hotel data."));